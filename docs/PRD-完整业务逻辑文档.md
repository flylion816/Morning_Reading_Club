...
> **文档版本**: v6.1（完整业务逻辑 + 测试覆盖版 + 近期修复补充）
> **创建日期**: 2026-03-03
> **最后更新**: 2026-03-06
> **基于代码**: GitHub 最新提交，包含 1,597 个自动化测试 + E2E Playwright 测试
...
### 1.3 近期修复与优化（v6.1, 2026-03-04 ~ 2026-03-06）

#### 点赞功能完善
- ✅ **修复**: 评论（Comment）和回复（Reply）的点赞/取消点赞功能完整实现
- ✅ **修复**: 前后端数据模型对齐，用户 ID 字段统一使用 `_id`
- ✅ **修复**: 嵌套回复（nested reply）的点赞状态正确同步
- ✅ **修复**: 点赞计数（likeCount）在前端即时更新，无需刷新页面

#### 打卡与课程页面修复
- ✅ **修复**: 首页期次卡片显示正确的用户打卡天数
- ✅ **修复**: 课程详情页加载空白问题（完整 periodId 结构传递）
- ✅ **修复**: 未支付用户点击课程时引导至支付页面，而非显示空白
- ✅ **修复**: 打卡统计数据准确性

#### 小程序资源优化
- ✅ **优化**: 分享图片（`share-default.png`、`share-insight.png`）压缩至 200KB 以内
- ✅ **说明**: 使用 JPEG 80% 质量替代原始 PNG，从 235KB/252KB 压缩至约 30KB
- ✅ **目的**: 解决微信开发者工具打包上传时的资源超限警告

#### 开发环境配置
- ✅ **修复**: `.env.config.js` 中 MongoDB 开发环境连接地址使用 `127.0.0.1` 替代 `localhost`
- ✅ **修复**: `backend/.env` 中 Redis 端口从生产环境的 `26379` 修正为本地的 `6379`
- ✅ **修复**: MySQL 和 Redis 的 HOST 从 `localhost` 改为 `127.0.0.1`（避免 IPv6 解析问题）

#### 脚本与工程清理
- ✅ **删除**: `scripts/fix-mocha-tests.sh`（一次性迁移脚本，已无用）
- ✅ **删除**: `scripts/tools/` 目录（临时工具脚本，已无用）
- ✅ **保留**: `scripts/deploy-to-server.sh`（生产部署脚本）
- ✅ **保留**: `scripts/generate-env-production.js`（环境配置生成）
- ✅ **保留**: `scripts/install-test-hooks.sh`（Git hooks 安装）

#### E2E 端到端测试基础设施
- ✅ **新增**: `tests/e2e/admin-ui.py` — 管理后台 Playwright 自动化测试（5 个场景，100% 通过）
- ✅ **新增**: `tests/e2e/miniprogram-ui.py` — 小程序 UI 测试脚本（需微信开发者工具 CDP 支持）
- ✅ **新增**: `tests/e2e/e2e-workflow.py` — 端到端完整工作流测试
- ✅ **说明**: 管理后台 E2E 测试覆盖登录、仪表板、报名管理、小凡看见管理、用户列表
- ⚠️ **限制**: 小程序 E2E 测试受微信开发者工具 CDP 协议限制，暂无法自动化执行
...
#### 3.7.3 点赞与取消点赞逻辑（v6.1 修复）

```
用户点击点赞按钮
    │
    ├─【场景1】对小凡看见（Insight）点赞
    │   ├─ POST /api/v1/insights/:id/like
    │   ├─ 后端检查是否已点赞
    │   │   ├─ 未点赞 → 新增点赞记录，likeCount +1
    │   │   └─ 已点赞 → 取消点赞记录，likeCount -1
    │   └─ 前端即时更新：
    │       ├─ likeCount 数字变化
    │       └─ 点赞按钮高亮/取消高亮
    │
    ...
    │   ├─ 后端检查 userId 是否已在 likes 数组中
    │   │   ├─ 未点赞 → push userId, likeCount +1
    │   │   └─ 已点赞 → pull userId, likeCount -1
    │   └─ 前端使用 _id（非 id）匹配用户
    │
    └─【场景3】对回复（Reply，嵌套在评论中）点赞
        ├─ POST /api/v1/comments/:commentId/replies/:replyId/like
        ├─ 后端在 comment.replies 数组中找到指定 reply
        │   ├─ 未点赞 → push userId, reply.likeCount +1
        │   └─ 已点赞 → pull userId, reply.likeCount -1
        └─ 前端使用 _id 字段判断 isLikedByMe

设计要点:
  ✅ 用户 ID 全局统一使用 _id（Mongoose ObjectId）
  ✅ 前端通过 app.globalData.userInfo._id 获取当前用户ID
  ✅ likes 数组存储用户 _id 列表，用于判断 isLikedByMe
  ✅ 点赞/取消点赞为同一接口，后端自动切换（toggle）
  ✅ 嵌套回复点赞通过三段式路由实现
```
...
| v6.0 | 2026-03-04 | 完整业务逻辑文档、测试体系整合 |
| v6.1 | 2026-03-06 | **点赞完善、图片压缩、脚本清理、E2E 测试、开发环境修复** ✅ |
