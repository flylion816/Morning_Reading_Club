...
**重要设计补充**:
- ✅ 首页期次卡片显示用户在该期次的正确打卡天数（通过 `GET /api/v1/periods/user` 获取个人数据）
- ✅ 课程详情页传递完整的 periodId 结构，避免加载空白
- ✅ 未支付用户点击课程时自动引导至支付页面，而非显示空白
...
- ✅ 分享图片（`share-default.png`、`share-insight.png`）压缩至 200KB 以内（JPEG 80% 质量，从 235/252KB 压缩至约 30KB）
- ✅ 所有小程序资源文件均控制在 200KB 以内，符合微信开发者工具打包上传规范
...
**本地开发环境要求**:
- MongoDB、MySQL、Redis 均在本地运行
- `.env.config.js` 中 `currentEnv = 'dev'` 切换到开发环境
- 数据库 HOST 使用 `127.0.0.1`（避免 IPv6 解析问题）
- Redis 本地端口为 `6379`（生产环境为 `26379`）
- 运行 `npm run init:dev` 初始化开发数据库（MongoDB + MySQL 表结构 + 索引 + 课程内容）

**工程脚本**:
- `scripts/deploy-to-server.sh` — 生产部署脚本
- `scripts/generate-env-production.js` — 环境配置生成（从 `.env.config.js` 生成 `.env.docker` 和 `.env.production`）
- `scripts/install-test-hooks.sh` — Git hooks 安装（pre-commit、pre-push）
...
本项目拥有企业级的完整测试体系，包括手工验收测试、自动化单元测试和 E2E 端到端测试。
...
- **范围**:
  ...
  - v6.1 新增：18 个测试用例（点赞、资源优化、E2E、开发环境）
...
- **管理后台**: `tests/e2e/admin-ui.py` — 5 个场景，覆盖登录、仪表板、报名管理、小凡看见管理、用户列表（100% 通过）
- **小程序**: `tests/e2e/miniprogram-ui.py` — 需微信开发者工具 CDP 支持（受环境限制）
- **完整流程**: `tests/e2e/e2e-workflow.py` — 端到端工作流测试（小程序 + 管理后台联动）
