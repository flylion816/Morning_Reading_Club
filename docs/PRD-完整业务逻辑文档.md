...
**认证流程**:
1. 管理员访问 `/admin` 路由，进入登录页 (`LoginView`)
2. 输入邮箱 + 密码，调用 `POST /api/v1/auth/admin/login`
3. 后端验证凭证，返回 JWT token
4. 前端存储 token，使用 `Authorization: Bearer <token>` 请求受保护资源
5. Token 过期时调用 `POST /api/v1/auth/admin/refresh` 自动刷新
...
**功能概述**: 管理员登录后进入仪表板，展示关键业务指标的实时概览。

**核心指标**:
1. **总报名数** - 所有期次的报名总人数
2. **待审批数** - 需要管理员审批的报名记录数
3. **支付总额** - 已支付报名的累计金额
4. **活跃期次** - 当前进行中的期次数量

**仪表板组件**:
- 数据卡片: 显示上述四个关键指标
- 趋势图表: 按时间维度显示报名人数/收入趋势
- 最近报名列表: 显示最近 10 条报名记录，包含:
  - 用户昵称、报名期次、报名时间、支付状态、操作按钮(查看详情/审批/拒绝)

**技术实现**:
- 调用 `GET /api/v1/insights/dashboard` 获取数据
- 数据实时更新，用户可手动刷新或设置自动刷新间隔

### 6.3 期次与内容管理

**期次管理** (`PeriodsView`):
- 显示所有期次列表，包含: 期次名称、开始时间、结束时间、报名人数、价格、发布状态
- 支持新建期次: 输入期次名称、开始/结束时间、价格、描述信息
- 支持编辑期次: 修改上述信息(仅在期次开始前)
- 支持发布开关: 勾选 "已发布" 使期次在小程序首页可见
- 数据操作:
  - `POST /api/v1/periods` - 新建期次
  - `PUT /api/v1/periods/:id` - 编辑期次
...
2. 显示该期次的课节列表，包含:
   ...
   - 发布状态 (已发布/未发布)
   - 当前期次的打卡数
3. 支持新增课节: 输入标题、内容、发布状态
4. 支持编辑课节: 修改上述信息
5. 支持删除课节
6. 数据操作:
   - `POST /api/v1/content` - 新建课节
   - `PUT /api/v1/content/:id` - 编辑课节
   - `DELETE /api/v1/content/:id` - 删除课节
   - `GET /api/v1/content/:periodId` - 获取该期次的课节列表

### 6.4 用户与报名管理

**用户管理** (`UsersView`):
- 显示所有用户列表，包含: 用户ID、昵称、邮箱、账号状态(正常/禁用)
- 支持按昵称/邮箱搜索用户
- 支持修改用户状态 (禁用/启用)
- 数据操作:
  - `GET /api/v1/users` - 获取用户列表
  - `PUT /api/v1/users/:id` - 修改用户状态

**报名管理** (`EnrollmentsView`):
1. 显示所有报名记录，包含:
   - 用户昵称、报名期次、报名时间、支付状态、状态(待审批/已同意/已拒绝)
2. 支持搜索: 按用户名/期次名称搜索
3. 支持筛选: 按报名状态、支付状态筛选
4. 支持查看详情: 点击报名记录进入详情页，显示用户信息、报名理由、是否已支付
5. 支持操作:
   - 审批: 将 "待审批" 状态改为 "已同意"，用户即可进入课程
   - 拒绝: 将 "待审批" 状态改为 "已拒绝"，向用户发送拒绝通知
   - 批量删除: 选择多个报名记录删除
6. 数据操作:
   - `GET /api/v1/enrollments` - 获取报名列表
   - `PUT /api/v1/enrollments/:id` - 审批/拒绝报名
...
**支付管理** (`PaymentsView`):
- 显示所有支付记录，包含:
  - 订单号、用户昵称、期次名称、支付金额、支付方式(微信/支付宝)、支付状态(已支付/待支付/退款)、支付时间
- 支持多维筛选:
  - 按订单号搜索
  - 按支付状态筛选 (已支付/待支付/退款)
  - 按支付方式筛选 (微信/支付宝)
...
**打卡管理** (`CheckinsManagementView`):
1. 显示打卡统计信息:
   - 今日打卡数 (当天内的打卡数)
   - 总打卡数 (全站的打卡总数)
2. 显示打卡记录列表，包含:
   - 用户昵称、期次名称、打卡日期、打卡内容(摘要)、点赞数、评论数
3. 支持多维筛选:
   - 按用户昵称搜索
   - 按期次筛选
   - 按时间范围筛选
4. 支持查看详情: 点击打卡记录查看完整内容、点赞用户列表、评论列表
5. 支持删除: 管理员可删除违规打卡记录
6. 数据操作:
...
1. 先在下拉菜单中选择一个期次
2. 显示该期次的小凡看见列表，包含:
   - 发布人昵称、发布时间、状态(已发布/已下架)、点赞数、评论数
3. 支持操作:
   - 发布: 将 "草稿" 状态改为 "已发布"，小程序用户可见
   - 下架: 将 "已发布" 状态改为 "已下架"，小程序用户不可见
4. 支持管理员编辑:
   - 新增小凡看见: 输入发布人、期次、标题、内容
   - 编辑小凡看见: 修改上述信息
   - 删除小凡看见
5. 数据操作:
   - `GET /api/v1/insights/:periodId` - 获取该期次的小凡看见列表
   - `POST /api/v1/insights` - 新增小凡看见
   - `PUT /api/v1/insights/:id` - 编辑小凡看见
   - `DELETE /api/v1/insights/:id` - 删除小凡看见
   - `PATCH /api/v1/insights/:id/publish` - 发布小凡看见
   - `PATCH /api/v1/insights/:id/unpublish` - 下架小凡看见

**查看申请审批** (`InsightRequestsManagementView`):
1. 显示所有的小凡看见申请，包含:
   - 申请人昵称、申请类型(新增/编辑)、申请内容(摘要)、申请时间、审批状态(待审批/已同意/已拒绝)
2. 支持多维筛选:
   - 按申请状态筛选 (待审批/已同意/已拒绝)
   - 按申请人昵称搜索
3. 支持操作:
...
4. 支持统计:
   ...
   - 同意率 (同意申请数 / 总申请数)
5. 数据操作:
   - `GET /api/v1/insight-requests` - 获取申请列表
   - `PATCH /api/v1/insight-requests/:id/approve` - 同意申请
   - `PATCH /api/v1/insight-requests/:id/reject` - 拒绝申请

### 6.8 审计日志

**审计日志** (`AuditLogsView`):
1. 显示所有管理后台的操作记录，包含:
   - 操作员(管理员邮箱)、操作类型(查看/创建/修改/删除/发布/审批)、操作对象(期次/报名/支付等)、操作时间、操作详情(IP、User-Agent等)
2. 支持多维筛选:
   - 按操作员搜索
   - 按操作类型筛选 (查看/创建/修改/删除/发布/审批)
   - 按操作对象筛选 (期次/报名/支付/用户等)
   - 按时间范围筛选
3. 支持导出: 将筛选结果导出为 CSV 或 Excel
4. 支持统计:
   - 按操作类型统计操作频次
   - 按操作员统计操作次数
   - 按时间维度显示操作趋势
...
**数据库管理** (`DatabaseView`):
- 仅限管理员访问，需要二次密码认证 (输入管理员密码)
- 包含两个子页面:

**MongoDB 数据查看**:
1. 选择集合 (collections): users、periods、enrollments、checkins、insights 等
2. 显示该集合的所有文档
3. 支持查询:
   - 输入 MongoDB query (JSON 格式)
   - 例: `{"status": "paid"}` 查询所有已支付的报名
4. 支持导出: 将查询结果导出为 JSON

**MySQL 数据同步**:
1. 显示同步状态: 最后同步时间、同步记录数、同步成功/失败
2. 支持手动触发同步:
   - 全量同步: 从 MongoDB 导出全部数据到 MySQL (用于报表分析)
   - 增量同步: 仅同步新增/修改的数据
3. 显示同步日志
