...
| sections_{periodId} | Array | 课节列表（按期次） | 课程页 |
| checkins_{periodId} | Array | 打卡列表（按期次） | 打卡广场 |
...
async function checkIfCheckinExists(localCheckin) {
  try {
    ...
    // 检查是否有相同内容的打卡
    return res.data.items.some(c => 
      c.sectionId === localCheckin.sectionId &&
      c.content === localCheckin.content
    );
  } catch (error) {
    return false;
  }
...
async function getPeriods() {
  return apiFirst(
    () => request({ url: '/periods', method: 'GET' }),
    'periods',
    {
...
Page({
  ...
  async loadCheckins(page = 1) {
    ...
    try {
      ...
      const checkins = page === 1 
        ? res.data.items 
        : [...this.data.checkins, ...res.data.items];
