...
> **当前维护说明（2026-05-12）**
> 本文档中的统计数字是历史基线。当前新增页面级回归测试包括：
> `miniprogram/__tests__/pages/course-detail.spec.js`（打卡详情长图模板懒生成、长文完整/降级策略）
> 和 `miniprogram/__tests__/pages/insight-detail.spec.js`（小凡看见海报列表保留、加粗边界换行）。
> 后续更新统计数字前，应以 `npm test -- --listTests` 或实际测试运行结果重新统计。
...
| 文件 | 用例数 | 覆盖点 |
|------|-------|--------|
| **insight.request-flow.test.js** | 3 | 同条目重复申请复用、单条授权不外溢到整期、他人列表按条锁定/解锁 |
