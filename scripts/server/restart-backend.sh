...
# 7. 显示详细状态
log_info "详细状态信息："
pm2 show "$APP_NAME" | grep -E "(status|memory|cpu|uptime)" | sed 's/^/  /'

# 8. 显示最近日志
log_info "最近日志（最后10行）:"
if pm2 logs "$APP_NAME" --lines 10 --nostream 2>/dev/null | head -10 | sed 's/^/  /'; then
  :
else
  log_warning "无法显示日志"
fi
