...
describe('Comment Model', () => {
  ...
  describe('Schema Validation', () => {
    it('应该创建有效的评论', async () => {
      ...
      expect(comment.targetId.toString()).to.equal(targetId.toString());
      expect(comment.targetType).to.equal('Checkin');
    ...
    it('应该使用默认值', async () => {
      ...
      expect(comment.status).to.equal('active');
      expect(comment.likeCount).to.equal(0);
    });
  ...
  describe('Data Persistence', () => {
    it('应该保存并检索评论数据', async () => {
      ...
      expect(retrieved.likeCount).to.equal(10);
    });

    it('应该支持按targetId和targetType查询', async () => {
      ...
      const comments = await Comment.find({ targetId: t1, targetType: 'Checkin' });
