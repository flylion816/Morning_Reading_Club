...
> - ✅ 数据库备份验证
>
> 详见 [完整部署清单](./.Codex/memory/deployment-checklist.md) 和 [部署指南](./DEPLOYMENT.md)
...
**⭐ 遇到问题？首先查看**：[`.Codex/memory/quick-reference.md`](./.Codex/memory/quick-reference.md)
...
5. **Memory 系统**（🆕 新增）→ [`.Codex/memory/`](./.Codex/memory/)
   - 快速问题查询：[`quick-reference.md`](./.Codex/memory/quick-reference.md)
   - 使用指南：[`README.md`](./.Codex/memory/README.md)
   - 按问题类型分类：frontend、backend、architecture
   - **推荐作为首选查询方式**

6. **快速命令系统**（🆕 新增）→ [`.Codex/commands/`](./.Codex/commands/)
   - 开发命令：启动服务
   - 测试命令：快速测试 API 和功能
   - 搜索命令：查询 Memory 系统
   - 部署命令：检查部署和备份
   - **使用指南**：[`.Codex/commands/README.md`](./.Codex/commands/README.md)

7. **Git Hooks 自动化**（🆕 新增）→ [`.Codex/hooks/`](./.Codex/hooks/)
   - Pre-commit：代码质量检查
   - Commit-msg：提交信息验证
   - Post-commit：自动反馈
   - **使用指南**：[`.Codex/hooks/README.md`](./.Codex/hooks/README.md)

8. **Subagents 专家代理**（🆕 新增）→ [`.Codex/agents/`](./.Codex/agents/)
   - 前端专家：小程序开发
   - 后端专家：API和业务逻辑
   - 数据库专家：MongoDB优化
   - 部署专家：版本和运维
   - **使用指南**：[`.Codex/agents/README.md`](./.Codex/agents/README.md)
   - **性能提升**：并行处理，减少50%开发时间
...
本项目实现了一个完整的四阶段优化系统，使 Codex 开发效率提升 **300%+**。新会话应立即了解这些工具的用途和使用方法。

### 📊 四个阶段一览

| 阶段  | 名称           | 文件位置            | 用途                                | 新会话如何使用                                             |
| ----- | -------------- | ------------------- | ----------------------------------- | ---------------------------------------------------------- |
| **1** | 🧠 Memory 系统 | `.Codex/memory/`   | 知识库：历史问题和解决方案索引      | 遇到问题？先查 `.Codex/memory/quick-reference.md`         |
| **2** | 🔧 Git Hooks   | `.Codex/hooks/`    | 自动化：代码质量和提交验证          | 自动执行，无需手动操作                                     |
| **3** | ⚡ Commands    | `.Codex/commands/` | 快速命令：9个常用脚本               | 按需执行：`./Codex/commands/development/start-backend.sh` |
| **4** | 🤖 Subagents   | `.Codex/agents/`   | 专家代理：4个领域专家并行处理大任务 | 大功能用 Subagents，小任务直接做                           |
...
```bash
# 优先级1：查询 Memory 快速参考
.Codex/memory/quick-reference.md

# 优先级2：搜索历史问题
.Codex/commands/search/search-bug.sh "你的问题关键词"
...
# 第二步：启动服务
.Codex/commands/development/start-all.sh      # 启动所有服务（推荐）
# 或分别启动：
.Codex/commands/development/start-backend.sh  # 仅启动后端
.Codex/commands/development/start-miniprogram.sh # 仅启动小程序

# 第三步：验证开发环境
.Codex/commands/testing/test-api.sh            # 测试 API
.Codex/commands/testing/test-auth.sh           # 测试认证
```
...
步骤：
  1️⃣ 在 Memory 查询类似问题：.Codex/commands/search/search-bug.sh "样式"
  2️⃣ 修改代码
...
```bash
.Codex/commands/deployment/check-deploy.sh     # 部署检查清单
.Codex/commands/deployment/backup-db.sh        # 数据备份

查看详情：.Codex/agents/deployment-expert.yaml
```
...
| 您需要...           | 查看此文件                                   |
| ------------------- | -------------------------------------------- |
| 🔥 快速解决问题     | `.Codex/memory/quick-reference.md`          |
| 📚 了解所有Commands | `.Codex/commands/README.md`                 |
| 🪝 了解Git Hooks    | `.Codex/hooks/README.md`                    |
| 🤖 了解Subagents    | `.Codex/agents/README.md`                   |
| 🧠 理解Memory系统   | `.Codex/memory/README.md`                   |
| 💻 Commands使用经验 | `.Codex/memory/standards/commands-usage.md` |
...
# 第3步：查询快速参考（如果有疑问）
cat .Codex/memory/quick-reference.md
```
...
```bash
# 启动后端服务
.Codex/commands/development/start-backend.sh

# 启动小程序（在微信开发工具中）
.Codex/commands/development/start-miniprogram.sh
```
...
```bash
# 快速测试所有主要 API
.Codex/commands/testing/test-api.sh

# 测试认证流程
.Codex/commands/testing/test-auth.sh

# 测试打卡功能
.Codex/commands/testing/test-insights.sh
```
...
```bash
# 搜索历史问题
.Codex/commands/search/search-bug.sh "页面空白"
.Codex/commands/search/search-bug.sh "用户ID"

# 按类别浏览
.Codex/commands/search/search-bug.sh frontend  # 前端问题
.Codex/commands/search/search-bug.sh backend   # 后端问题
```
...
```bash
# 检查部署状态
.Codex/commands/deployment/check-deploy.sh

# 备份数据库
.Codex/commands/deployment/backup-db.sh
```

**详见** → [`.Codex/commands/README.md`](./.Codex/commands/README.md)
...
1. **🧠 Memory 系统** (Stage 1) - 完成
   - 知识库索引：`.Codex/memory/quick-reference.md` (200+行)
   - 问题分类：frontend/backend/architecture
   - 搜索脚本：`.Codex/commands/search/search-bug.sh`

2. **🔧 Git Hooks 自动化** (Stage 2) - 完成
   - Pre-commit：代码质量检查
   - Commit-msg：提交信息验证
   - Post-commit：自动反馈
   - 安装脚本：`.Codex/hooks/install.sh`

3. **⚡ Commands 快速命令系统** (Stage 3) - 完成
   - 9个可执行脚本：开发/测试/搜索/部署
   - 详细文档：`.Codex/commands/README.md` (450+行)
   - 使用经验：`.Codex/memory/standards/commands-usage.md` (360+行)
...
- 将 4708 行的 AGENTS.md 拆分为 4 个专题文档
...
### 32. Codex 上下文清理经验 - 避免会话间的进程泄漏 (2025-11-30)

**问题现象**：Codex 环境使用越来越慢，`/context` 命令显示：
