# 小凡看见查看申请功能 - 完整设计方案

## 📋 功能概述

用户可以向他人发起查看其"小凡看见"记录的申请。被申请者可以同意或拒绝。同意后，申请者可以查看被申请者在该期次的所有小凡看见。

---

## 🗄️ 数据模型设计

### 1. InsightRequest（查看申请表）

**用途**：记录用户之间的查看申请

**字段设计**：

```javascript
{
  _id: ObjectId,

  // 申请双方
  fromUserId: ObjectId (ref User),      // 申请者
  toUserId: ObjectId (ref User),        // 被申请者

  // 申请状态
  status: String (enum: ['pending', 'approved', 'rejected']),

  // 申请信息
  reason: String (optional),             // 申请原因
  createdAt: Date,
  updatedAt: Date,

  // 审批信息
  approvedAt: Date (optional),           // 同意时间
  rejectedAt: Date (optional),           // 拒绝时间

  // 权限范围
  periodId: ObjectId (ref Period),       // 允许查看的期次（仅approved时有效）

  // 索引
  indexes: {
    unique: { fromUserId, toUserId },   // 同一对用户只能有一个pending申请
    composite: { toUserId, status },    // 按被申请者和状态查询
    composite: { fromUserId, status },  // 按申请者和状态查询
    createdAt: -1                        // 按创建时间排序
  }
}
```

**说明**：
- `unique` 索引防止重复申请
- `periodId` 仅在 `status='approved'` 时有意义
- 支持多期查看：如果需要多期，可扩展为 `periodIds: [ObjectId]`

---

## 🔗 关系表设计

### InsightPermission（权限表）- 可选

如果需要更细粒度的权限控制，可使用此表：

```javascript
{
  _id: ObjectId,

  // 关系
  granteeId: ObjectId (ref User),       // 被授予权限者（申请者）
  grantorId: ObjectId (ref User),       // 授予权限者（被申请者）

  // 权限范围
  periodId: ObjectId (ref Period),      // 可查看的期次
  insights: [ObjectId] (ref Insight),   // 可查看的specific insights（可选）

  // 状态
  status: String (enum: ['active', 'revoked']),

  // 时间
  grantedAt: Date,                      // 授予时间
  expiresAt: Date (optional),           // 过期时间
  revokedAt: Date (optional),           // 撤销时间

  // 索引
  indexes: {
    composite: { granteeId, grantorId, periodId },
    composite: { grantorId, status }
  }
}
```

**说明**：
- 当 InsightRequest.status 改为 'approved' 时，创建对应的 InsightPermission 记录
- 支持权限过期、撤销等高级功能

---

## 🔌 API 接口设计

### 1️⃣ 发起查看申请

**请求**：
```
POST /api/v1/insights/requests
Authorization: Bearer {token}

{
  "toUserId": "用户ID",
  "reason": "想看看你的学习反馈" (optional)
}
```

**响应成功（201）**：
```json
{
  "code": 200,
  "data": {
    "_id": "申请ID",
    "fromUserId": "申请者ID",
    "toUserId": "被申请者ID",
    "status": "pending",
    "createdAt": "2025-12-04T12:00:00Z"
  },
  "message": "申请已发送"
}
```

**错误情况**：
- 400: 自己不能向自己发起申请 → "无需向自己发起查看请求"
- 409: 已有pending申请 → "申请已存在，请等待对方回复"
- 200: 已有approved申请 → "已获得查看权限"

---

### 2️⃣ 获取收到的申请列表

**请求**：
```
GET /api/v1/insights/requests/received?status=pending
Authorization: Bearer {token}
```

**响应**：
```json
{
  "code": 200,
  "data": [
    {
      "_id": "申请ID",
      "fromUserId": {
        "_id": "用户ID",
        "nickname": "阿泰",
        "avatarUrl": "...",
        "avatar": "🦁"
      },
      "status": "pending",
      "reason": "想看看你的学习反馈",
      "createdAt": "2025-12-04T12:00:00Z"
    }
  ],
  "message": "获取成功"
}
```

---

### 3️⃣ 同意查看申请

**请求**：
```
PUT /api/v1/insights/requests/{requestId}/approve
Authorization: Bearer {token}

{
  "periodId": "期次ID"  // 本期的periodId
}
```

**逻辑**：
1. 验证当前用户是申请的 `toUserId`
2. 验证申请状态为 'pending'
3. 更新申请状态为 'approved'
4. 创建 InsightPermission 记录（如使用）
5. 记录同意时间

**响应（200）**：
```json
{
  "code": 200,
  "data": {
    "_id": "申请ID",
    "status": "approved",
    "periodId": "期次ID",
    "approvedAt": "2025-12-04T12:30:00Z"
  },
  "message": "已同意查看请求"
}
```

---

### 4️⃣ 拒绝查看申请

**请求**：
```
PUT /api/v1/insights/requests/{requestId}/reject
Authorization: Bearer {token}
```

**逻辑**：
1. 验证当前用户是申请的 `toUserId`
2. 验证申请状态为 'pending'
3. 更新申请状态为 'rejected'
4. 记录拒绝时间

**响应（200）**：
```json
{
  "code": 200,
  "data": {
    "_id": "申请ID",
    "status": "rejected",
    "rejectedAt": "2025-12-04T12:35:00Z"
  },
  "message": "已拒绝查看请求"
}
```

---

### 5️⃣ 获取发起的申请列表

**请求**：
```
GET /api/v1/insights/requests/sent?status=all
Authorization: Bearer {token}
```

**响应**：
```json
{
  "code": 200,
  "data": [
    {
      "_id": "申请ID",
      "toUserId": {
        "_id": "用户ID",
        "nickname": "狮子",
        "avatarUrl": "..."
      },
      "status": "approved",
      "createdAt": "2025-12-04T12:00:00Z",
      "approvedAt": "2025-12-04T12:30:00Z",
      "periodId": "期次ID"
    }
  ]
}
```

---

### 6️⃣ 获取他人的小凡看见（需要权限检查）

**请求**：
```
GET /api/v1/insights/user/{userId}/period/{periodId}
Authorization: Bearer {token}
```

**权限检查逻辑**：
1. 如果 userId === 当前用户 → 允许查看自己的
2. 如果存在 approved InsightRequest 且 periodId 匹配 → 允许查看
3. 否则 → 403 Forbidden

---

## 🔄 完整流程示例

### 场景：阿泰向狮子发起查看申请

#### 1️⃣ 阿泰发起申请
```
POST /insights/requests
{ toUserId: "狮子ID", reason: "想看看你的学习反馈" }

↓ 创建 InsightRequest 记录
{
  fromUserId: "阿泰ID",
  toUserId: "狮子ID",
  status: "pending"
}
```

#### 2️⃣ 狮子在首页看到申请
```
GET /insights/requests/received?status=pending

↓ 返回待审批的申请列表
[{ fromUserId: "阿泰...", status: "pending", ... }]
```

#### 3️⃣ 狮子同意查看
```
PUT /insights/requests/{requestId}/approve
{ periodId: "2025年12月期次ID" }

↓ 更新记录
{
  status: "approved",
  periodId: "2025年12月期次ID",
  approvedAt: "2025-12-04T12:30:00Z"
}

↓ 创建权限记录（可选）
InsightPermission: {
  granteeId: "阿泰ID",
  grantorId: "狮子ID",
  periodId: "2025年12月期次ID",
  status: "active"
}
```

#### 4️⃣ 阿泰可以查看狮子本期的小凡看见
```
GET /insights/user/狮子ID/period/2025年12月期次ID

↓ 权限检查通过，返回狮子在本期的所有小凡看见
```

#### 5️⃣ 如果拒绝
```
PUT /insights/requests/{requestId}/reject

↓ 更新记录
{
  status: "rejected",
  rejectedAt: "2025-12-04T12:35:00Z"
}

↓ 在阿泰的"请求看见"列表中显示"拒绝"状态
```

---

## 📊 前端数据流

### 页面1：他人主页（profile-others）
```
点击"小凡看见" → 检查是否已有权限 →
  ├─ 已批准 → 显示"查看小凡看见"
  ├─ 待批准 → 显示"等待对方回复"
  └─ 未申请/被拒 → 弹窗确认发起申请
```

### 页面2：首页（index）- 新增"请求看见"板块
```
显示收到的申请列表：
[
  { 昵称: "阿泰", 时间: "2小时前", 状态: "待审批" }
]

点击申请行 → 弹窗显示申请详情 →
  ├─ 点击"✓" → 同意 → 后台调用approve接口
  └─ 点击"✗" → 拒绝 → 后台调用reject接口
```

---

## ✅ 数据一致性保证

| 场景 | 处理方式 |
|------|--------|
| 同一对用户多个pending申请 | 利用unique索引防止 |
| 申请被删除后用户仍在权限列表 | 权限系统独立管理 |
| 期次删除后权限仍存在 | 可考虑cascade delete |
| 用户被禁用 | 禁用其所有待审批申请 |

---

## 🔐 权限检查清单

- [ ] 发起申请：验证不是自己
- [ ] 审批申请：验证是被申请者
- [ ] 查看小凡看见：验证权限
- [ ] 撤销权限：只有被授予者或授予者可撤销

---

## 📈 可选扩展

1. **权限过期**：approved 后30天过期
2. **批量授权**：一次同意多个期次
3. **权限撤销**：被授予者可以取消权限
4. **申请提醒**：实时通知被申请者
5. **黑名单**：可拒绝特定用户的后续申请
6. **审计日志**：记录所有权限变更

